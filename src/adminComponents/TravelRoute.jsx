...
const TravelRoute = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/TravelRoute/addRoute`, routeData, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  return (
    <>
      ...
      <Sidebar />
      <Box sx={{ p: 10, ml: 30 }}>
        <Typography variant="h5" gutterBottom>Add Travel Route</Typography>
          ...
          <Select
            ...
            renderValue={(selected) => (
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                {selected.map((id) => (
                  <Chip key={id} label={autos.find(a => a._id === id)?.vehicleName || id} />
                ))}
              </Box>
            )}
          >
        ...
        <FormControl fullWidth margin="normal" error={Boolean(errors.carIds)}>
          ...
          <Select
            ...
            renderValue={(selected) => (
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                {selected.map((id) => (
                  <Chip key={id} label={cars.find(c => c._id === id)?.vehicleName || id} />
                ))}
              </Box>
            )}
          >
